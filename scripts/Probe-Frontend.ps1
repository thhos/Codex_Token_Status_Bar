...
& (Join-Path $framework 'csc.exe') /nologo /platform:x64 /codepage:65001 "/win32manifest:$projectRoot\src\windows\app.manifest" "/reference:$target\CodexPetCredits.exe" "/out:$target\FrontendProbe.exe" @references (Join-Path $projectRoot 'tests\FrontendProbe.cs')
if ($LASTEXITCODE -ne 0) { throw 'Probe build failed' }
Copy-Item -LiteralPath (Join-Path $target 'CodexPetCredits.exe.config') -Destination (Join-Path $target 'FrontendProbe.exe.config') -Force
