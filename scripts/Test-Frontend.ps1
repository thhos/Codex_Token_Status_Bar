...
& (Join-Path $framework 'csc.exe') /nologo /platform:x64 /codepage:65001 "/reference:$target\CodexPetCredits.exe" "/out:$target\FrontendRegression.exe" @references (Join-Path $projectRoot 'tests\FrontendRegression.cs')
